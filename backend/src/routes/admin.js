...
router.post("/users/:id/make-admin", adminAuth, async (req, res) => {
  try {
    ...
    // user.role = "admin"; // Assuming you have a role field in your User model
    await user.save();
...
router.post("/users/:id/remove-admin", adminAuth, async (req, res) => {
  try {
    ...
    // user.isSuperuser = true
    user.role = "user"; // Assuming you have a role field in your User model
    await user.save();

    res.json({ message: "User is remove from admin" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Server error" });
  }
